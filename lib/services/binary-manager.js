...
class BinaryManager extends EventEmitter {
  async init() {
    ...
    try {
      ...
      logger.log({ level: 'error', msg: `Binary-Manager | Failed to retrieve ${suggestedVersion ? suggestedVersion : 'the latest version'}`});
    }
    ...
    setInterval(async () => {
      ...
      try {
        ...
        logger.log({ level: 'error', msg: `Binary-Manager | Failed to retrieve the latest version`});
        return;
      }
      ...
      try {
        ...
        logger.log({ level: 'error', msg: `Binary-Manager | Failed to retrieve ${suggestedVersion}`});
      }
    }, 5 * 60 * 1000);
  }
  ...
  getDownloadUrl(version) {
    switch (platform()) {
      case 'win32': return `https://github.com/storj/storj/releases/download/v${version}/storagenode_windows_amd64.zip`;
      case 'linux': return `https://github.com/storj/storj/releases/download/v${version}/storagenode_linux_amd64.zip`;
      default: throw new Error(`Unsupported platform: ${platform()}`);
    }
  }

  getDownloadBinaryNameRegex() {
    switch (platform()) {
    ...
    try {
      await decompress(
        ...
        config.binaryDirectory, {
          filter: file => this.getDownloadBinaryNameRegex().test(file.path),
          map: file => {
