...
class BinaryManager extends EventEmitter {
  async init() {
    ...
    this.latestVersion = await this.retrieveSuggestedVersion();
    await this.getForVersion(this.latestVersion);
  ...
  getVersionBinPath(version) {
    const binaryName = `storagenode-${version}.exe`;
  ...
  async downloadBinary({ version }) {
    ...
    const url = `https://github.com/storj/storj/releases/download/v${version}/storagenode_windows_amd64.exe.zip`;
    ...
    try {
      await decompress(
        ...
        config.binaryDirectory, {
          ...
          map: file => {
            file.path = `storagenode-${version}.exe`;
