...
class Config {
  ...
  get staggeredStartDelayInSeconds() {
    return this.config.staggeredStartDelayInSeconds;
  }
  ...
  async load() {
    ...
    try {
      ...
      await this.save();
      logger.log({ level: 'info', msg: `Config | Default config written to ${this.configPath}, exiting ..`});
      process.exit();
    }
  ...
  get defaultConfig() {
    return {
      ...
      nodes: [{
        name: 'Node 01',
        configDir: join(homedir(), '.config', 'storj-manager', 'nodes', 'node-01'),
      }],
    };
  }
}
